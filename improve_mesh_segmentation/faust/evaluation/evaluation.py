...
def compute_correction_statistics(dataset_path, segmentation_labels_path, logging_dir, corrections_path):
    ...
    # load corrections
    if corrections_path.endswith(".npy"):
        corrections = np.load(corrections_path)
    elif corrections_path.endswith(".csv"):
        corrections = pd.read_csv(corrections_path, header=None).to_numpy()
    ...
    for mesh_idx, ((_, gt_o), (_, gt_c)) in tqdm(enumerate(zip(noisy_dataset, clean_dataset))):
        ...
        # Create array that indicates which vertices are to be corrected according to method
        to_be_corrected = np.zeros((gt_o.shape[0]))
        mesh_corrections = corrections[corrections[:, 0] == mesh_idx]
        to_be_corrected[mesh_corrections[:, 1]] = mesh_corrections[:, -1]
    ...
    print(
        f"Corrected Vertices: {corrections_counter}, "
        f"Noise Precision: {noise_precision}, "
        f"Noise Recall: {noise_recall}, "
        f"Noise F1: {noise_f1}"
    )
    return noise_precision, noise_recall, noise_f1, corrections_counter


def count_mislabeled_points(dataset_path, segmentation_labels_path, logging_dir):
    # Get noisy labels
    noisy_dataset = FaustSegmentationDataset(
        path_to_zip=dataset_path,
        path_to_segmentation_labels=segmentation_labels_path,
    ...
    total_mislabeled = 0
    for mesh_idx, ((_, gt_o), (_, gt_c)) in tqdm(enumerate(zip(noisy_dataset, clean_dataset))):
        # Count mislabeled points for mesh
        mesh_mislabeled = torch.sum(gt_o != gt_c)

        # Add to total count
        total_mislabeled += mesh_mislabeled
    return int(total_mislabeled)


def count_correction_suggestions(corrections_path):
    # load corrections
    if corrections_path.endswith(".npy"):
        corrections = np.load(corrections_path)
    elif corrections_path.endswith(".csv"):
        ...
        raise ValueError("The 'corrections_path' must end with .npy or .csv!")
    if len(corrections.shape) > 2:
        return int(corrections[:, -1].sum())
    else:
        return int(corrections.shape[0])
