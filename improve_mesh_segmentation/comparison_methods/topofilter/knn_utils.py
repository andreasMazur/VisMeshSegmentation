...
from PythonGraphPers import PyPersAll
...
def calc_knn_graph(feats_point_cloud, k=2, refer_trunk_size=10000, query_trunk_size=2000, progress=None):
    """
    ...
    with torch.no_grad():
        ...
        knnG = []
        task = progress.add_task("[cyan]Computing knn graph...", total=num_query_trunk)
...
def calc_topo_weights_with_components_idx(ntrain, prob_all, feats_point_cloud, ori_label, pred_label,
                                          use_log=False, nclass=2, k=2, cp_opt=3,
                                          refer_trunk_size=10000, query_trunk_size=2000, progress=None):
    """
    Since GPU knn is memory intensive, so we split the query and reference data points into several trunks.
    Each time, we process a trunk of data (in other words, a batch of data).
    ...
    cprint('computing chunked knn graph', 'yellow')
    start = timeit.default_timer()

    if progress is None:
        progress = Progress()
    
    knnG_list = calc_knn_graph(feats_point_cloud, k=k, refer_trunk_size=refer_trunk_size, query_trunk_size=query_trunk_size, progress=progress)

    stop = timeit.default_timer()
    cprint(f'Finished computing knn graph. Consumed time: {stop - start}', 'yellow')
    ...
    task = progress.add_task('[cyan]Computing topology...', total=nclass)
    start = timeit.default_timer()
