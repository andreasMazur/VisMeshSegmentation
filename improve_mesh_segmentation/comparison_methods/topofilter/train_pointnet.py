...
def train(
    ...
    for epoch in range(n_epochs):
        ...
        model.train()
        for step, ((data, _), labels, _) in enumerate(curr_train_set):
            data, labels = torch.unsqueeze(data.to(device).transpose(0,1), dim=0), labels.to(device)
            if data.size(2) == 0 or data.size(2) == 1:
                continue  # skip empty data
            optimizer.zero_grad()
            try:
                pred = torch.squeeze(model(data)[0], dim=0)
            except Exception as e:
                print(f'Error at step {step} in epoch {epoch}: {e}')
                breakpoint()
                pass
            ...
            epoch_accuracy = epoch_accuracy + multiclass_accuracy(pred, labels).detach()
            ...
            correct_verts += (predicted_classes == labels).sum().item()
            total_verts += labels.size(0)
        ...
        with torch.no_grad():
            for step, ((data, _), _, labels) in enumerate(val_dataset):
                data, labels = torch.unsqueeze(data.to(device).transpose(0,1), dim=0), torch.from_numpy(labels).to(device)
                pred = torch.squeeze(model(data)[0], dim=0)
                loss = F.nll_loss(pred, labels)
        ...
        if epoch >= when_to_denoise and (epoch - when_to_denoise) % denoise_every_n_epoch == 0:
            features_all = np.zeros((full_set_verts, 128))
            ...
            curr_idx = 0
            for step, ((data, _), labels, true_labels) in enumerate(train_dataset):
                data, labels = torch.unsqueeze(data.to(device).transpose(0,1), dim=0), labels.to(device)
                probs, feats = model(data)
            ...
            knnG_list = calc_knn_graph(feats_big_comp, k=k_outlier, progress=knn_progress)#cprint
            ...
            # clean_data_num = train_set_ignore_noisy.num_verts
            ...
            if clean_data_num > best_num_clean_labels:
                ...
                # Save model
                torch.save(model.state_dict(), f'./trained_models/pointnet_{when_to_denoise}_{denoise_every_n_epoch}_{k_outlier}_{k_cc}_{zeta}_tmp.pth')
                ...
                for every in everys:
                    for start_clean in start_cleans:
                        for zeta in zetas:
                            ...
                            knn_panel.title = f'Filtering progress for pointnet_{start_clean}_{every}_{k_outlier}_{k_cc}_{zeta}'
                            train_panel.title = f'Train progress for pointnet_{start_clean}_{every}_{k_outlier}_{k_cc}_{zeta}'
