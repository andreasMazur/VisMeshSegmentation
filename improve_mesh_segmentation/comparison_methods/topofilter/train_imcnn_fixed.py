...
def train(
    ...
    for epoch in range(n_epochs):
        ...
        if epoch >= when_to_denoise and (epoch - when_to_denoise) % denoise_every_n_epoch == 0:
            ...
            np.save(f"fixed_clearing/results/imcnn_stats_{when_to_denoise}_{denoise_every_n_epoch}_{k_outlier}_{k_cc}_{zeta}.npy", stats)

    df = train_dataset.get_results_from_noisy_data_indices(best_noisy_data_indices)
    df.to_csv(f'fixed_clearing/results/imcnn_corrections_{when_to_denoise}_{denoise_every_n_epoch}_{k_outlier}_{k_cc}_{zeta}.csv', header=False, index=False)
    best_model = Path(f'./fixed_clearing/trained_models/imcnn_{when_to_denoise}_{denoise_every_n_epoch}_{k_outlier}_{k_cc}_{zeta}_tmp.pth')
...
if __name__ == "__main__":
    ...
    k_ccs = [
       #5, 25, 100, 500#, 2000, 4000
       10 
    ]

    start_cleans = [2,3] #[1, 2, 3, 4]
                ...
                for every in everys:
                    for start_clean in start_cleans:
                        for zeta in zetas:
                            ...
                            csv_path = Path(f'./fixed_clearing/results/imcnn_corrections_{start_clean}_{every}_{k_outlier}_{k_cc}_{zeta}.csv')
                            if csv_path.exists():
